...
void GenericTCPApplication::handleMessage(cMessage *msg)
{
	...
	{
		if (strcmp("CodeRed", msg->getName())==0)
		{
			TCPSocket *socket = socketMap.findSocketFor(msg);

			char m[100];
			strcpy(m, "CodeRedRecieved");
			strcat(m, socket->getRemoteAddress().str().c_str());
			strcat(m, " infects ");
			strcat(m, socket->getLocalAddress().str().c_str());
			strcat(m, " at time " );
			strcat(m, simTime().str().c_str());

			cMessage *StateMsg=new cMessage(m);
			send(StateMsg,"stateOut");
		}
