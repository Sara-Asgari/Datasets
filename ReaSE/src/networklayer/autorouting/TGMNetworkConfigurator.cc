...
void TGMNetworkConfigurator::initialize(int stage)
{
	...
	for (unsigned int i = 0; i < asNodeVec.size(); i++)
	{
		...
		// print IP address of all hosts of network
		for (int j = 0; j < rlTopology[i]->getNumNodes(); j++)
		{
			if ( (rlTopology[i]->getNode(j)->getModule()->getFullName())[0]=='h' )
			{
				IInterfaceTable *ift = IPAddressResolver().findInterfaceTableOf(rlTopology[i]->getNode(j)->getModule());

				for (int32 k=0; k<ift->getNumInterfaces(); k++)
				{
					if (!ift->getInterface(k)->isLoopback()&&ift->getInterface(k)->ipv4Data()!=NULL)
					{
						std::ofstream ip_file;
						ip_file.open ("D:\\ip_file.txt", std::ofstream::out | std::ofstream::app);
						ip_file << "Ip address of "<< rlTopology[i]->getNode(j)->getModule()->getFullName()<<"  is   "  << ift->getInterface(k)->ipv4Data()->getIPAddress()<< endl;
						ip_file.close();
						cout << "Ip address of "<< rlTopology[i]->getNode(j)->getModule()->getFullName()<<"  is   "  << ift->getInterface(k)->ipv4Data()->getIPAddress()<< endl;
					}
				}
			}
		}
