 ...
 */
void TrafficProfileManager::getTrafficProfile(TrafficProfile &t, double profileProb, double foreignServerProb, bool &isForeign)
{
	double cum_prob = 0.;
	unsigned int i = 0;
...
void TrafficProfileManager::initialize(int stages)
{
	...
	if (profiles.size() == 0)
		opp_error("No traffic profiles specified for generation of background traffic");
