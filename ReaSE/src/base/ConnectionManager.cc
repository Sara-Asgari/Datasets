...
void ConnectionManager::getServer(TargetInfo &ti, TrafficProfile &tp, int &except, int moduleId)
{
	...
	if (except == -1)
	{
		foreignServerProb = uniform(0, 1, 0);
		profileProb = uniform(0, 1, 0);
		tpm->getTrafficProfile(tp, profileProb, foreignServerProb, isForeign);
	}
	else if (except == -2) // added by me. when we set traffic profile previously
	{
		foreignServerProb = uniform(0, 1, 0);
		profileProb = uniform(0, 1, 0);
...
void ConnectionManager::getForeignServer(TargetInfo &ti, int profileId, int moduleId)
{
	...
	while (goOn && runs < 100)
	{
		...
		if (UNIFORM_PROB == 1)
		{
			networkProb = uniform(0, subCM.size(), 0);
			i = floor(networkProb);
		}
		else
		{
			networkProb = uniform(0, 1, 0);
			cumProb = 0.;
...
bool ConnectionManager::haveProfile(TargetInfo &ti, int profileId, int moduleId)
{
	...
	while (goOn && runs < 100)
	{
		...
		if (UNIFORM_PROB == 1)
		{
			serverProb = uniform(0, svector->size(), 0);
			i = floor(serverProb);
		}
		else
		{
			cumProb = 0.;
			serverProb = uniform(0, 1, 0);
