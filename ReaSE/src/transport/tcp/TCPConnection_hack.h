...
protected:
	...
	bool worker;
public:
	TCPConnection_hack(TCP *mod, int appGateIndex, int connId):TCPConnection(mod, appGateIndex, connId) {this->worker = false;};
	~TCPConnection_hack();
protected:
	...
	void setWorkerThread() { this->worker = true; };
	/// @brief Returns if this connections is a worker thread or not
	bool isWorkerThread() { return worker; };
