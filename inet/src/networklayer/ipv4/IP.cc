...
void IP::handlePacketFromNetwork(IPDatagram *datagram)
{
	...
	// if destination of packet is router, drop it
	if ( (getParentModule()->getParentModule()-> getFullName())[0]!='h')
	{
	    for (int i=0; i<ift->getNumInterfaces(); i++)
	    {
	    	InterfaceEntry *ie = ift->getInterface(i);
	    	if(datagram->getDestAddress()==ie->ipv4Data()->getIPAddress())
	    	{
	    		EV << "destination of packet is not valid\n";
	    		delete datagram;
	    		return;
	    	}
	    }
...
void IP::reassembleAndDeliver(IPDatagram *datagram)
{
    ...
    {
        int gateindex = mapping.getOutputGateForProtocol(protocol);
        if(!strcmp(datagram->getName(),"worm_udp"))
        {
        	gateindex=7;
        }
